...
- Mumble TLS 控制连接、证书验证与 SHA-256 指纹固定、加密 UDP 语音和 TCP tunnel 回退
- 服务器收藏、Bonjour 局域网发现、公共服务器目录和延迟检测
- 频道树、频道管理、ACL、用户管理、频道切换、返回上一频道和文字聊天
- 按键说话、语音活动检测和持续发送模式
- 可视化麦克风电平、VAD 阈值与噪声底自动校准
- Opus 编解码、抖动缓冲、RNNoise 降噪、AGC、基础回声消除和可选的系统 VoiceProcessingIO
...
生成 Developer ID 签名并经过 Apple 公证的发布 `.dmg`：

```bash
DEVELOPER_ID_APP="Developer ID Application: Your Name (TEAMID)" \
NOTARY_PROFILE="SwiftMumble" \
  ./scripts/release-app.sh
```

`NOTARY_PROFILE` 是通过 `xcrun notarytool store-credentials` 预先保存在 Keychain 中的
公证凭据名称。发布产物位于 `.build/release/SwiftMumble.dmg`。
...
- [SwiftSoup](https://github.com/scinfu/SwiftSoup)：不可信 Mumble HTML 消息的白名单清洗
- [KeyboardShortcuts](https://github.com/sindresorhus/KeyboardShortcuts)：全局快捷键注册与录制
- [swift-certificates](https://github.com/apple/swift-certificates)：客户端 X.509 证书生成
- [TouchBarHelper](https://github.com/ddddxxx/TouchBarHelper)：Touch Bar 控件集成
